...
chit-chatting, just the basics: code, comments and exercises! Oh well, we will give some details concerning the first steps; and sometimes we will link to useful
...
IDE), but I highly recommend it. Go to the 
...
[webinar series](https://www.rstudio.com/collections/rstudio-essentials/). 
...
1 +                      # Incomplete commands can occupy several lines. In the console, the
  2                      # usual letter > is replaced by + to indicate an incomplete command.
...
* Make a numeric vector that repeats the sequence of even numbers between 
1 and 10 two times, with each number repeated three times. Use `rep` and `seq` for the solution. You may have to consult the R help. The expected output is:
`2  2  2  4  4  4  6  6  6  8  8  8 10 10 10  2  2  2  4  4  4  6  6  6
8  8  8 10 10 10`.
* Plot the functions sine and a cosine for $x = [0, 2 \pi]$ in the same figure.
Use different colors for each function.
* What's the output of `c(1, 3, 5, 7) + c(3, -2)`? And `c(2, 4, 1) * 3`? Try
to answer before testing it on the console.
* Gauss is known for solving the following problem in a few seconds. "Write down all the whole numbers from 1 to 100 and add up their sum". Do the same, storing
the numbers in the variable `numbers` and summing them with a function.
...
# ... and this is "character" (check this results on your own!)
...
* If `x <- c(TRUE, TRUE, FALSE, FALSE)` and `y <- c(TRUE, FALSE)`, what's the
output of `x & y`? and `x && y`? Try to predict the outputs before testing them.
* Imagine you have the vector `x <- c(5, 1, 8, 9, 15, 1, 24, 2, 14, 3)` and you
want to find out how many values are greater or equal than 9. Proceed as
follows: 
    1. The operator `>` is vectorized, so you can directly ask `x > 9` in the 
console. Try to predict the output before actually executing it.
    2. Cast the result from the previous step to numeric values. Again, try to 
    predict the output before executing it.
    3. Use `sum` to compute the final answer.
    4. If you didn't, try to express all the previous code in a single line!
* What's the output of `(!TRUE || (3 <= 4) ) && (5 %in% 1:10)`?
* `(TRUE + TRUE) * FALSE`, what does this expression evaluate to and why?
* If `fn <- "James"` and `ln <- "Bond"`, write code to produce as output 
`"Bond, James Bond"`. You can only use `fn` and `ln`. Pay attention to the comma!
* Use `paste` to generate the output `5 Mississippi, 10 Mississipi, ..., 50 Mississipi` (Note that we are only using multiples of 5!).
* Consider `names <- c('Torvalds', 'Gauss', 'Einstein')` and 
`jobs <- c('Computer Scientist', 'Mathematician', 'Physicist')`. Write code
to get the exact output 
`Torvalds is a Computer Scientist, Gauss is a Mathematician, Einstein is a Physicist'`.
...
```{r vectors}
# Set seed is useful for obtaining the same results when using random
# functions like sample (see below)
set.seed(1)
...
tail(grades, -3)            # All but the first three values
```
...
## Programming Challenges (III)
* Consider the vector  `x <- c(1,0,-3, NA, NaN, -Inf, 4)`, What is the value of
`(x+2)[!is.na(x) & (x > 0)]`?
* Which values of the previous vector `x` fulfilling that the exponential of `x`
is greater than 0.1?
* Given `x <- c(6, 7, 1, 9, 5, 10, 8, 4, 2, 3)`, create a new vector `y` by
eliminating the first 2 numbers and the last 2 numbers. Now sum the first three
numbers of `y` with the second set of three numbers from `y`. The expected
output is `11 17 9`.
* If `x <- c(2, 4, 6, 8)` and `y <- c(TRUE, FALSE, TRUE, FALSE)`, what will be the output for the R expression `max(x[y])`? 
...
cat(grades_mat[, "algebra"], "==", grades_mat[, 1])  # By columns: grades from algebra
cat(grades_mat[3, ], "==", grades_mat["Maria", ])           # By rows: grades of Maria
cat(grades_mat[3, 1], "==", grades_mat["Maria", "algebra"]) # Both: grades of Maria in algebra
...
## Programming Challenges (IV)
* Create the matrix $\begin{bmatrix}
2 & 4 & 3\\
1 & 5 & 7
\end{bmatrix}$ using the `matrix` function.
* Create a grades matrix named `grades` for the following data: 
`students <- c("Sven", "Otto", "Hedda", "Hilda")`,
`algebra_grades <- sample(1:10, 4, replace = TRUE)`,
`programming_grades <- sample(1:10, 4, replace = TRUE)` and
`calculus_grades <- sample(1:10, 4, replace = TRUE)` using `cbind`. Add names
for the rows and the columns.
* Add the grades of Greta (10 in algebra, 10 in programming and a 9 in calculus)
to the `grades` matrix from the previous exercises. Use the `rbind` function.
* Subset the grades of Hedda and Greta in algebra and calculus.
* Find the worst grade for each student using `apply`.
* Find the mean grade for each subject. Also find the mean grade for each
student.
* Let's extract the first column of the `grades` matrix. Compare the output
of `grades[, 1]` with the output of `grades[, 1, drop = FALSE]`. What's the
difference?
* Let's say that you have a grade "C" if your numeric grade is smaller than 5,
a "B" if your grade is in the range $[5,8)$, and an "A" if your grade is equal
or greater than 8. Transform the `grades` matrix into a character matrix using
conditional indexing (That is, you may use things like 
`grades[grades == 0] <- "zero"`).
...
# In the same spirit of subset (to avoid using grades_df$...), we have transform and with
transform(grades_df, is_clever = (grade >= 7))    # add a new variable
with(grades_df, sex == "Male")                   # manipulations to a variable
```

```{r}
# It is common that grades_dfa.frames contain information about several groups.
# We can study them separately using split:
...
## Programming Challenges (V)
* Transform the `grades` matrix from *Programming Challenges (IV)* into
a data frame, also including information about the sex of the student. The 
resulting data frame should have columns `name`, `algebra`, `programming`,
`calculus` and `sex`. The type of the columns should be numerical for the grades,
character for `name`, and factor for `sex`.
* Compute the mean grades for `males` and also for `females`.
* Use `subset` for finding out which students have an algebra grade greater 
than 7. 
* Use `transform` to create a new column named `average` with the average
grades.
* Consider the next data frame. Create it, save it as the variable `df`, and calculate the percentage of unemployed (in this case, 50%).
...
* If `x <- 1:3`, `y <- c('a', 'b')` and `l <- list(alpha = x, beta = y)`, what
is the result of `l[2]`? What about `l[[2]]`? Is there any difference? Can you
write down other three ways to access this data?
* What happens if (using the list from the previous exercise) you execute
`l$alpha <- NULL`?  . Try to predict before testing the code.
* Use `unlist` to compute the sum of the elements from 
`l <- list ("x"=5, "y"=10, "z"=15)`.
* Transform the `grades` matrix from *Programming Challenges (IV)* into
a list, also including information about the sex of the student. The 
resulting list should have items named `name`, `algebra`, `programming`,
`calculus` and `sex`. 
* Transform the list from the previous exercise into a data frame. Then, transform
back the resulting data frame into another list.
...
## Programming Challenges (VI)
* What does the function `my_sum <- function(x, y) { z <- x + y }` return? 
Can you improve the previous function?
* Create the function `is_in(element, vector)` that returns TRUE if 
`element` is in `vector`.
* Create the function `count_elem(elem, vector)` that returns how many times 
`elem` appears inside `vector`.
* Create a function that given a matrix, it returns the matrix with all the
zeros changed to `Inf`.
...
## Programming Challenges (VII)
* Create a `list()` with ten elements using a for loop. To generate the i-th 
element (the element in the i-th position of the list) do the following:
    1. Generate a random vector of length i using `runif`.
    2. If the mean of the random vector is greater than 0.5, then store in the
    i-th element of the list the character `"it is greater!"`. If not, store 
    `"it is not greater"`.
* Repeat the previous exercise avoiding the use of the for loop by means of 
`lapply`. You surely will have to create a custom function first.
* Imagine that you want to generate two different random numbers from the 
set `c(1, 2, 3)`. That is, valid results are `c(1, 2)`, `c(3,1)`, `c(3,2)`, etc.
Invalid results would be `c(1, 1)`, `c(2, 2)` and `c(3, 3`). We can achieve this
quite easily by using `sample(1:3, 2)`; however we shall implement it using 3
different approaches `for` loops, `repeat` and `while` (just for practicing!, 
the `sample` approach is better). To that end, follow the next algorithm:
    1. Generate a first number using `sample(1:3, 1)`. 
    2. Generate a second random number.
    3. If the two numbers are the same, go back to 1.
* Remember the `with` function in the data frames section? we had
`with(grades_df, split(grades, sex))`. Use `lapply` to compute the mean grade 
of each sex. 
...
S3 is quite primitive and lacks a formal definition, but
...
## Programming Challenges (VIII)
* Implement a specialized implementation of `mean` for the class 
`mega_cool_vector`. This function should not only compute the mean, but it should
also print on screen `"The mega cool mean is [result]"`.
...
```{r, echo=FALSE}
# Make an empty chart
plot(1, 1, xlim = c(1, 5.5), ylim = c(0, 6), type = "n",
...
plot(x, y, xlab = "x", ylab = "exp(0.5 * x)", 
     main = "Second function", type = "b")
...
with(mtcars, boxplot(mpg), xlab = "Number of Cylinders",
...
## Programming Challenges (IX)
* Load the `cars` data and plot a scatterplot of it. Change the color to blue, 
and use a plotting character (`pch`) different from the default value. Finally,
change the x-label, y-label and title of the plot.
* Suppose you want to highlight results from the previous plot; let's say, you
want to identify the point with the largest speed. First find which datum has
the larger speed (you should get `speed = 25`, `dist = 85`). Use the `points` function to add a new point to the previous plot, using the red color to identify
it.
* Using the `iris` dataset, make a boxplot of `Sepal.Width` stratified by 
`Species`.
* Use a barplot to count the number of flowers within each species in the 
`iris` dataset.
...
In this section, we use several files stored under the `data` folder of the
GitHub repository.
...
## Programming Challenges (X)
* Try to read the files 'iris.csv', 'iris_2.csv', 'iris_3.csv', 'iris.tsv' and
'iris_2.tsv' from the data folder. It is a good idea to first open the data
with a text editor to see the delimiters and the format of the file.
* Save the `iris` dataset using both CSV and TXT files.